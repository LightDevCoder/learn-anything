...
If source material is sparse, return `BLOCKED` with the exact missing method fields. Treat `TBD`, `TODO`, `N/A`, `unknown`, empty placeholders, generic template boilerplate, inline literal markup such as `<placeholder>`, and embedded unresolved markers such as `TBD - decide later` as missing evidence even when headings are present. Treat an asserted resource ending in an unresolved marker as a source gap; an explicit `none` resource is allowed. If invocation evidence names both user-invoked and model-invoked, return a blocked invocation-type conflict instead of choosing one. If it is narration, a one-off task, or a passive summary, return a learning summary explicitly marked `not_promoted`. Do not draft a package until source evidence is sufficient.
