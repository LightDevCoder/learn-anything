...
If source material is sparse, return `BLOCKED` with the exact missing method fields. Treat `TBD`, `TODO`, `N/A`, `unknown`, empty placeholders, generic template boilerplate, and embedded unresolved markers such as `TBD - decide later` as missing evidence even when headings are present. Treat an asserted resource ending in an unresolved marker as a source gap; an explicit `none` resource is allowed. If it is narration, a one-off task, or a passive summary, return a learning summary explicitly marked `not_promoted`. Do not draft a package until source evidence is sufficient.

## Decision Workflow

1. Identify whether the source is a reusable workflow, narration, a passive summary, a one-off task, or too sparse to assess. Use affirmative title, leading-context, Purpose, or Trigger evidence for one-off and passive-summary classification; do not demote a complete method because a field says `not merely a one-off`, `no longer a one-off`, or because only a failure mode records `no procedure`.
